...
export const Authentication:React.FC<IProps> = ({onClose, setEmailHeader, setLogIn}) => {
    ...
    const signUp = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if(signInVisible) {
            ...
            && nameError === '' && emailError === '' && surnameError === '' && passwordError === '') {
           ...
           if(user.email === JSON.parse(localStorage.getItem(user.email)!)?.email) {
             if(user.password === JSON.parse(localStorage.getItem(user.email)!)?.password) {
                   setEmailHeader(user.email)
                   onClose()
                   setLogIn(true)
    ...
    return (
        <Modal showModal={onClose}>
            ...
            <div className={styles.inputCont}>
                ...
                <button className={styles.signInButton} onClick={signUp}>{signInVisible ? 'Sign Up': 'Sign In'}</button>
